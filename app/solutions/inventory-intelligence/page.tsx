...
import {
  ...
  Cpu, Wrench, Shield, CheckCircle2, AlertTriangle, Truck, Zap,
  Package, Factory, Layers, Activity, Tag, ClipboardList, FlaskConical, Box, Gem, Barcode,
  Cloud, Brain
} from "lucide-react";
import Header from "@/components/Header";
import Footer from "@/components/Footer";

export default function InventoryIntelligenceSolutionPage() {
  ...
  const useCases = [
    {
      ...
      img: "/images/supply-chain/supply_chain_case_distribution.png",
      href: "/resources/case-studies/real-time-distribution-center-inventory",
      overview: "Warehouses manage thousands of products moving through receiving, storage, picking, packing, and shipping operations. Lack of inventory visibility can lead to fulfillment delays, inventory inaccuracies, and operational inefficiencies. Delonti's Inventory Intelligence Platform provides real-time visibility into inventory across warehouse operations.",
      benefits: ["Visibility", "Utilization", "Efficiency", "Compliance"]
    },
    {
      title: "Retail Stock Tracking",
      img: "/images/supply-chain/supply_chain_case_retail.png",
      href: "/resources/case-studies/omnichannel-retail-fulfillment",
      overview: "Retailers must maintain accurate inventory levels across stores, stockrooms, distribution centers, and online channels to support customer demand and omnichannel fulfillment. Delonti enables retailers to gain complete inventory visibility from shelf to warehouse.",
      benefits: ["Visibility", "Utilization", "Efficiency", "Compliance"]
    },
    {
      title: "Healthcare Supplies Management",
      img: "/images/healthcare/healthcare_case_inventory.png",
      href: "/resources/case-studies/healthcare-inventory-optimization",
      overview: "Healthcare organizations rely on accurate inventory visibility to ensure critical supplies and medical consumables are available when needed for patient care. Delonti's Inventory Intelligence Platform helps healthcare providers manage supplies efficiently while reducing waste and improving compliance.",
      benefits: ["Visibility", "Utilization", "Efficiency", "Compliance"]
    },
    {
      title: "Manufacturing Materials Management",
      img: "/images/supply-chain/supply_chain_case_reverse.png",
      href: "/resources/case-studies/reverse-logistics-optimization",
      overview: "Manufacturers require real-time visibility into raw materials, work-in-progress inventory, and finished goods to support production schedules and reduce operational risk. Delonti provides complete inventory intelligence across the manufacturing lifecycle.",
      benefits: ["Visibility", "Utilization", "Efficiency", "Compliance"]
    }
  ];
    ...
    <main className="w-full bg-[#FAFAFA] dark:bg-background-dark font-sans selection:bg-logo selection:text-white relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
            ...
            >
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black mb-6 tracking-tighter leading-[1.1] text-slate-900 dark:text-white uppercase">
                ...
                <span className="text-logo dark:text-blue-400">Visibility & Intelligence</span>
              </h1>

              <p className="text-sm md:text-base text-slate-500 dark:text-slate-400 mb-10 leading-relaxed max-w-xl font-medium">
                Track inventory across warehouses, facilities, and supply chains with RFID and AI-powered insights.
              </p>
            ...
            >
              <img
                src="/images/solutions/inventory_intelligence_hero.png"
                alt="Inventory Visibility and Intelligence"
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-12 lg:mb-16 text-center max-w-3xl mx-auto">
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tighter text-slate-900 dark:text-white uppercase">
              Why Inventory Management Is a Challenge
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { title: "Inventory Inaccuracies", desc: "Discrepancies between physical stock and system records." },
              { title: "Stockouts & Overstocks", desc: "Unbalanced supply causes replenishment bottlenecks and waste." },
              { title: "Manual Counting", desc: "Cycle counts rely on paper trails, manual logs, and audits." },
              { title: "Limited Site Visibility", desc: "No central real-time view of inventory across warehouses." }
            ].map((prob, i) => (
      ...
      <section className="py-20 lg:py-24 bg-[#FAFAFA] dark:bg-background-dark-alt border-y border-slate-200/60 dark:border-slate-855 relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="flex flex-col md:flex-row items-stretch justify-between gap-4 relative">
            {[
              { step: "RFID Tags", desc: "Affixed to inventory boxes & pallets", icon: Tag },
              { step: "Readers", desc: "Detect tag signals continuously", icon: Radio },
              { step: "Cloud", desc: "Central database consolidation", icon: Cloud },
              { step: "AI Analytics", desc: "Stock prediction algorithms", icon: Brain },
              { step: "Dashboard", desc: "Real-time replenishment visibility", icon: Monitor }
            ].map((node, idx, arr) => {
      ...
      <section className="py-16 lg:py-20 bg-white dark:bg-background-dark">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-row md:grid md:grid-cols-5 gap-4 md:gap-6 overflow-x-auto no-scrollbar pb-4 md:pb-0 snap-x snap-mandatory">
            {[
              { title: "Real-Time Inventory Tracking", desc: "Automated, live visibility of stock on shelves and racks." },
              { icon: Cpu, title: "Automated Counting", desc: "No manual audits, instant calculations across checkpoints." },
              { icon: Wrench, title: "Stock Level Monitoring", desc: "Notifications when inventory drops below safety thresholds." },
              { icon: Truck, title: "Movement Tracking", desc: "Detect shipping, receiving, and storage moves." },
              { icon: Monitor, title: "Reporting", desc: "Interactive inventory reports with utilization and turnover stats." }
            ].map((cap, i) => {
              const subtleThemes = [
                {
      ...
      <section className="py-16 lg:py-20 bg-slate-50/50 dark:bg-background-dark-alt border-t border-slate-200/60 dark:border-slate-800">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { val: "Improve Accuracy", desc: "Ensure real-time physical inventory levels align with databases." },
              { val: "Reduce Stockouts", desc: "Automate replenishments to prevent lost sales." },
              { val: "Automate Audits", desc: "Run daily virtual audits instead of manual cycle counts." },
              { val: "Optimize Supply Chain", desc: "Speed up logistics movement from receiving to final shipping." }
            ].map((benefit, idx) => (
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
            {[
              ...
              { label: "Raw Materials", icon: Factory },
              { label: "Components", icon: Layers },
              { label: "Medical Supplies", icon: Activity },
              { label: "Retail Merchandise", icon: Tag },
              { label: "MRO Inventory", icon: ClipboardList },
              { label: "Laboratory Supplies", icon: FlaskConical },
              { label: "Pallets & Containers", icon: Box },
              { label: "High-Value Inventory", icon: Gem },
              { label: "Lot & Serialized Inventory", icon: Barcode }
            ].map((inventory, idx) => {
      ...
      <section className="py-16 bg-slate-900 text-white border-t border-slate-855 relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-6 text-center">
            {[
              { val: "25–50%", label: "Faster Audits" },
              { val: "Up to 99%", label: "Inventory Accuracy" },
              { val: "Reduced", label: "Stockouts" },
              { val: "Lower", label: "Carrying Costs" },
              { val: "Improved", label: "Replenishment" },
              { val: "Enhanced", label: "Efficiency" },
